...
class TransferFundsViewModel: ObservableObject {
    ...
    var filterAccounts: [AccountViewModel] {
        ...
        if self.fromAccount == nil {
            ...
            return accounts.filter {
                ...
                return $0.accountId != self.fromAccount!.accountId
    ...
    var toAccountType: String {
        fromAccount != nil ? toAccount!.accountType : ""
    }
    ...
    func submitTransfer(completion: @escaping () -> Void) {
        ...
        guard let fromAccount = fromAccount, let toAccount = toAccount, let amount = Double(amount) else {
            return
        }
        
        let transferFundsRequest = TransferFundRequest(accountFromId: fromAccount.accountId.lowercased(), accountToId: toAccount.accountId.lowercased(), amount: amount)
        
        AccountService.shared.transferFunds(transferFundRequest: transferFundsRequest) { result in
            switch result {
